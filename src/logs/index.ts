...
class Logger {
  constructor() {
    ...
    process.stdout.write(greenBright(`[RBAC] ${yellow(process.pid)} - ${white(moment().format('DD/MM/YYYY hh:mm:ss A'))}, INFO ${Logger.name} Service Iniatialized \n`))
  }
  /**
  ...
  private HandleCreateLogger(level: LoggingLevel = "info") {
    return createLogger({
      format: combine(
        ...
        printf((info) => `[RBAC-API] ${info.level.toUpperCase()}, ${info.timestamp} - ${info.message}`)
      ),
  ...
  dev(text: string, type: LoggingLevel = "info") {
    ...
    if (type === "info") {
      return process.stdout.write(greenBright(`[RBAC] ${yellow(process.pid)} - ${white(moment().format('DD/MM/YYYY hh:mm:ss A'))}, ${(type).toUpperCase()} ${text} \n`))
    }
    if (type === "error") {
      return process.stdout.write(red(`[RBAC] ${process.pid} - ${white(moment().format('DD/MM/YYYY hh:mm:ss A'))}, ${(type).toUpperCase()} ${text} \n`))
    }
    if (type === "debug") {
      process.stdout.write(bold(`[RBAC] ${process.pid} - ${white(moment().format('DD/MM/YYYY hh:mm:ss A'))},${(type).toUpperCase()} ${text} \n`))
      return process.exit(1)
    }
    if (type === "alert") {
      process.stdout.write(magenta(`[RBAC] ${yellow(process.pid)} - ${white(moment().format('DD/MM/YYYY hh:mm:ss A'))}, ${(type).toUpperCase()} ${text}\n`))
    }
    if (type === "notice") {
      return process.stdout.write(yellow(`[RBAC] ${process.pid} - ${white(moment().format('DD/MM/YYYY hh:mm:ss A'))}, ${(type).toUpperCase()} ${text}\n`))
    }
  };
  /**
